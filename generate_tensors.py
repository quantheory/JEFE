...
os.makedirs('collision_data', exist_ok=True)
...
CTENS_TYPE = "Long"
FILE_NAME_TEMPLATE = os.path.join("collision_data", CTENS_TYPE + "_ctens_nb{}.nc")
...
if CTENS_TYPE == "Hall_ScottChen":
    ckern = bm.HallKernel(const, 'ScottChen')
elif CTENS_TYPE == "Long":
    ckern = bm.LongKernel(const)

for nb in BIN_NUMBERS:
    print(f"Creating collision tensor with {nb} bins.")
