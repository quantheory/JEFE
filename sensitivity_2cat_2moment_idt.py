...
EXP_FILE_NAME_TEMPLATE = \
    os.path.join("sensitivity_experiments",
                 "experiment_d{:02d}nu{:02d}_c0-3_r0-3_idt.nc")
...
perturbed_variables = [
    (wvc0, bm.IdentityTransform(), const.std_diameter**0 * scale),
    (wvc3, bm.IdentityTransform(), const.std_diameter**3 * scale),
    (wvr0, bm.IdentityTransform(), const.std_diameter**0 * scale),
    (wvr3, bm.IdentityTransform(), const.std_diameter**3 * scale),
]
# Subject to error equivalent to placement of:
...
perturbation_rate = np.diag(errors**2) / 3600.
