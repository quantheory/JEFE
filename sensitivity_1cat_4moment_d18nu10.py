...
# Moments that are always used.
FIXED_MOMENTS = [0, 3, 9]
# Moments to loop over.
CHECKED_MOMENTS = [4, 5, 7, 8, 2, 1]
...
for check_mom in CHECKED_MOMENTS:
    print("Starting loop for: ", check_mom, flush=True)
    ...
    # Run experiment.
    print("Starting integration for: ", check_mom, flush=True)
    try:
    ...
    exp_file_name = EXP_FILE_NAME_TEMPLATE.format(
        d_micron, nu, moments[0], moments[1], moments[2], moments[3])
