...
EXP_FILE_NAME_TEMPLATE = \
    os.path.join("sensitivity_experiments",
                 "experiment_aguset_c0-3r0-3_80micron_hardcase.nc")

# Initial conditions
INITIAL_MASS = 2.e-3 # Initial mass concentration (kg/m^3)
INITIAL_NC = 800. # Initial number concentration (cm^-3)
...
perturbed_vars = [
    ...
    bm.PerturbedVar('Mr0', wvr0, bm.QuadToLogTransform(x0s[2]), db_scale),
    bm.PerturbedVar('Mr3', wvr3, bm.QuadToLogTransform(x0s[3]), db_scale),
]
