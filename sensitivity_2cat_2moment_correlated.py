...
EXP_FILE_NAME_TEMPLATE = \
    os.path.join("sensitivity_experiments",
                 "experiment_d{:02d}nu{:02d}_correlated_c0-3_r0-3.nc")
...
perturbed_variables = [
    (wvc0, bm.QuadToLogTransform(x0s[0]), db_scale),
    (wvc3, bm.QuadToLogTransform(x0s[1]), db_scale),
    (wvr0, bm.QuadToLogTransform(x0s[2]), db_scale),
    (wvr3, bm.QuadToLogTransform(x0s[3]), db_scale),
]
# Rate of error growth.
rate_mag = db_error_rate**2 / 3600.
pn = len(perturbed_variables)
perturbation_rate = rate_mag * (1.e-6 * np.eye(pn) + np.ones((pn, pn)))
